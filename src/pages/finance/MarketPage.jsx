      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex items-start gap-3">
          ...
          <p className="text-sm text-amber-800 font-medium leading-relaxed">
            Ice Cream Inc is trending up this week. It may be a good time to buy.
          </p>
        </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-green-800">
                {selected.action === 'buy' ? 'Buying' : 'Selling'} <span className="font-bold">{selected.stock.name}</span> ({selected.stock.ticker})
              </p>
            </div>
            <button onClick={() => setSelected(null)} className="text-green-600 hover:text-green-800">
        ...
        <div className="bg-white rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {portfolio.stocks.map((stock) => (
              <div key={stock.id} className="flex items-center gap-3 px-4 py-3" style={{ borderLeft: `4px solid ${stock.color}` }}>
                ...
                <div className="flex gap-1.5 shrink-0">
                  <button onClick={() => handleSelect(stock, 'buy')} className="px-3 py-1.5 bg-primary text-white text-xs font-bold rounded-lg hover:bg-primary-dark transition-colors">
                    Buy
                  </button>
                  <button onClick={() => handleSelect(stock, 'sell')} className="px-3 py-1.5 bg-slate-100 text-slate-700 text-xs font-bold rounded-lg hover:bg-slate-200 transition-colors">
                    Sell
                  </button>
                </div>
              </div>
        ...
        <div className="bg-slate-100 border border-slate-200 rounded-2xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">This is a learning simulation. No real money involved.</p>
        </div>
      </div>
    </div>
  );
