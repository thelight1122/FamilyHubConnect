  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-100 sticky top-0 bg-white/80 backdrop-blur-md z-10">
        ...
        <button className="bg-[#4c8ce6] text-white p-2 rounded-full flex items-center justify-center hover:bg-[#3a7bd5] transition-colors">
      ...
      <div className="flex-1 pb-24">
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`size-10 rounded-lg ${c.iconBg} ${c.iconColor} flex items-center justify-center`}
                      >
                    ...
                    {c.status !== 'completed' && (
                      <button className="text-slate-400 hover:text-[#4c8ce6]">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-50">
                    ...
                    {c.status === 'pending_review' && (
                      <button className="text-xs font-bold bg-slate-100 px-3 py-1 rounded-full uppercase">
                        Mark Done
                      </button>
                    )}
                    {c.status === 'completed' && (
                      <p className="text-xs font-medium text-slate-400 italic">
                        Resolved {c.resolvedAgo}
                      </p>
                    )}
                  </div>
                  ...
                  {c.status === 'in_progress' && (
                    <div className="w-full bg-slate-100 h-1.5 rounded-full mt-3 overflow-hidden">
                      <div
                        className="bg-[#4c8ce6] h-full rounded-full"
        ...
        <section className="px-4 py-6">
              ...
              >
                <span
                  className={`material-symbols-outlined ${
                    h.status === 'completed' ? 'text-emerald-500' : 'text-slate-400'
                  }`}
                >
          ...
          <button className="w-full mt-4 py-3 border border-dashed border-slate-300 rounded-xl text-slate-500 text-sm font-medium hover:border-[#4c8ce6] hover:text-[#4c8ce6] transition-all">
            Load Older History
          </button>
        </section>
